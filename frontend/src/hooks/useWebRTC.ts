...
export function useWebRTC(options?: UseWebRTCOptions): UseWebRTCReturn {
  ...
  const start = useCallback(async () => {
    ...
    try {
      ...
      // ScriptProcessorNode is deprecated but kept for broad browser compat (AudioWorklet requires secure context + separate file).
      // Retained intentionally — browser will log a one-time deprecation warning, not an application error.
  ...
  const flushPendingCandidates = useCallback(async () => {
    ...
    for (const init of queued) {
      try {
        ...
        // Expected during trickle-ICE race (candidate before remoteDescription)
        console.debug("[webrtc] dropping queued ICE candidate:", err);
      }
    }
  }, []);
  ...
  const addIceCandidate = useCallback(
    async (candidate: RTCIceCandidateInit) => {
      ...
      try {
        ...
        // Non-fatal trickle-ICE race; downgrade to debug to avoid console-warning spam
        console.debug("[webrtc] addIceCandidate failed:", err);
      }
    },
    [],
  );
